...
from .processes import get_processes, ProcessPipelineBuilder
...
def trigger_process(mmut_id : str):
    ...
    pipeline : ProcessPipelineBuilder =  get_processes(mmut_path)

    # Start the flow in a background thread
    thread = threading.Thread(target=run_docker_flow_sync, args=(pipeline,))
    thread.daemon = True
    thread.start()
